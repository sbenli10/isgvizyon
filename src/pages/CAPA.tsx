...
export default function CAPA() {
  ...
  const previewEvidence = (url: string) => {
    void (async () => {
      const accessUrl = await resolveStorageObjectUrl(url);
      setPreviewEvidenceUrl(accessUrl || url);
    })();
  };
  ...
  const downloadEvidenceUrl = async (url: string) => {
    try {
      ...
      link.href = blobUrl;
      link.download = (accessUrl || url).split("/").pop() || "kanit-dosyasi";
