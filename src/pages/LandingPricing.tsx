  ...
  if (status === "limited") {
    return (
      <li className="flex items-start gap-3 text-sm text-amber-200">
        ...
        <div>
          <span className="font-medium">{label}</span>
          <span className="ml-2 text-amber-300/90">({value})</span>
        </div>
      </li>
    );
  }
  ...
  if (status === "excluded") {
    return (
      <li className="flex items-start gap-3 text-sm text-slate-500">
        ...
        <div className="line-through decoration-slate-600/80">{label}</div>
      </li>
    );
  }
...
export default function LandingPricing() {
  ...
  const planFeatureMap = {
    Ücretsiz: pricingComparisonRows.map((row) => ({
      label: row.feature,
      value: row.free,
    })),
    Premium: pricingComparisonRows.map((row) => ({
      label: row.feature,
      value: row.premium,
    })),
    OSGB: pricingComparisonRows.map((row) => ({
      label: row.feature,
      value: row.osgb,
    })),
  } as const;
    ...
    >
        ...
        <div className="relative z-10">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="text-sm text-slate-400">
              İSG süreçlerinizi daha görünür ve yönetilebilir hale getiren planlar
            </span>
          </div>
          ...
          <div className="mt-6 max-w-3xl">
            ...
            <p className="mt-4 text-sm leading-8 text-slate-300 md:text-base">
              Küçük ekiplerden çok firmalı OSGB yapılarına kadar, her kullanım modeli için daha net,
              daha karşılaştırılabilir ve daha güvenli bir plan yapısı.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-5 lg:grid-cols-3">
        {pricingPlans.map((plan, index) => {
          const audience =
            plan.title === "Ücretsiz"
              ? "Platformu tanımak isteyen ekipler"
              : plan.title === "Premium"
                ? "Aktif operasyon yürüten İSG profesyonelleri"
                : "Birden fazla firmayı yöneten OSGB yapıları";
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="flex h-11 w-11 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.05] text-cyan-200">
                      {index === 0 ? <ShieldCheck className="h-5 w-5" /> : <Sparkles className="h-5 w-5" />}
                    </div>
                    <h3 className="mt-4 text-2xl font-semibold tracking-[-0.04em] text-white">{plan.title}</h3>
                    <p className="mt-2 text-sm text-slate-400">{audience}</p>
                  </div>
                ...
                <div className="mt-6">
                  <div className="flex items-end gap-2">
                    ...
                    <span className="pb-1 text-sm text-slate-400">
                      {plan.price === "₺0" ? "/ sınırsız süre" : plan.title === "OSGB" ? "" : "/ aylık"}
                    </span>
                  </div>
                  <p className="mt-3 min-h-[72px] text-sm leading-7 text-slate-300">{plan.description}</p>
                </div>

                <ul className="mt-6 space-y-3">
                  {planFeatureMap[plan.title as keyof typeof planFeatureMap].slice(0, 7).map((feature) => (
      ...
      <section className="rounded-[32px] border border-[rgba(148,163,184,0.18)] bg-[rgba(15,23,42,0.76)] p-6 shadow-[0_24px_90px_rgba(0,0,0,0.24)] backdrop-blur-xl md:p-8">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h2 className="mt-3 text-2xl font-semibold tracking-[-0.04em] text-white md:text-3xl">
              Hangi özelliğin hangi planda açık olduğunu net görün
            </h2>
          </div>
          <p className="max-w-2xl text-sm leading-7 text-slate-300">
            Dahil olan özellikleri, kısıtlı kalan alanları ve yalnızca üst paketlerde açılan modülleri tek bakışta karşılaştırın.
          </p>
        </div>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          <div className="rounded-[22px] border border-white/10 bg-[#0B1F3A]/72 p-4">
            <p className="text-sm font-medium text-white">Güvenli ödeme akışı</p>
            <p className="mt-2 text-sm leading-7 text-slate-400">
              Paket akışınızı mevcut sistem yapınızla uyumlu şekilde sürdürebilirsiniz.
            </p>
          </div>
          <div className="rounded-[22px] border border-white/10 bg-[#0B1F3A]/72 p-4">
          ...
          <div className="rounded-[22px] border border-white/10 bg-[#0B1F3A]/72 p-4">
            <p className="text-sm font-medium text-white">İhtiyaca göre yükseltme</p>
            <p className="mt-2 text-sm leading-7 text-slate-400">
              Operasyon yoğunluğunuz arttıkça daha geniş planlara kontrollü şekilde geçebilirsiniz.
            </p>
          </div>
        </div>
      </section>
