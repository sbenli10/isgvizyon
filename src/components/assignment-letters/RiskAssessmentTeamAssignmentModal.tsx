...
export function RiskAssessmentTeamAssignmentModal({
  ...
  const handleCompanySelect = (companyId: string) => {
    ...
    const nextMembers: RiskAssessmentTeamMember[] = [
      ...
      { role: "Söndürme Ekip Başkanı", fullName: fields.emergencyTeams.fire[0]?.fullName || fields.fireSupportPersonName },
      { role: "Kurtarma Ekip Başkanı", fullName: fields.emergencyTeams.rescue[0]?.fullName || fields.evacuationSupportPersonName },
      { role: "Koruma Ekip Başkanı", fullName: fields.emergencyTeams.protection[0]?.fullName || "" },
      { role: "İlk Yardım Ekip Başkanı", fullName: fields.emergencyTeams.firstAid[0]?.fullName || fields.firstAidSupportPersonName },
    ].map((member, index) => ({
      ...member,
