...
export function FeatureAccessGate({
  ...
  if (loading) {
    return (
      <div className="rounded-[28px] border border-border bg-muted/50 p-6">
        <div className="animate-pulse space-y-4">
          <div className="h-5 w-40 rounded bg-muted" />
          <div className="h-4 w-72 rounded bg-muted" />
          <div className="grid gap-3 md:grid-cols-3">
            {Array.from({ length: 3 }).map((_, index) => (
              <div key={index} className="h-24 rounded-2xl border border-border bg-muted/70" />
            ))}
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
          <div className="max-w-3xl">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Badge className="rounded-full border border-border bg-card/80 px-3 py-1 text-muted-foreground">
                {plan === "premium" ? "Üyelik güncellemesi gerekiyor" : "Free planda kilitli"}
              </Badge>
            </div>
            <div className="mt-4 flex items-start gap-4">
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-card/80 text-foreground">
          ...
          <div className="min-w-[280px] rounded-[24px] border border-border bg-card/80 p-5">
