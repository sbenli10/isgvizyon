    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent overlayClassName="z-[100] bg-slate-950/80" className="z-[120] flex max-h-[92vh] max-w-[920px] flex-col overflow-hidden rounded-2xl border border-slate-700 bg-[#1b2638] p-0 text-slate-50 shadow-2xl [&>button.absolute]:hidden">
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-5">
          ...
          <div className="flex flex-wrap gap-2"><Button type="button" onClick={downloadTemplate} className="bg-slate-800 text-slate-100 hover:bg-slate-700"><Download className="mr-2 h-4 w-4" />Şablon Excel İndir</Button><label className="inline-flex h-10 cursor-pointer items-center rounded-xl border border-emerald-200 bg-emerald-300 px-4 text-sm font-black text-slate-950 shadow-sm shadow-emerald-950/20 transition hover:bg-emerald-200 focus-within:ring-2 focus-within:ring-emerald-300 focus-within:ring-offset-2 focus-within:ring-offset-slate-950"><Upload className="mr-2 h-4 w-4" />Excel Dosyası Seç<input type="file" accept=".xlsx,.xls" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) void parseFile(file); event.currentTarget.value = ""; }} /></label>{fileName ? <span className="inline-flex items-center rounded-xl border border-slate-700 px-3 text-sm text-slate-300">{fileName}</span> : null}</div>
